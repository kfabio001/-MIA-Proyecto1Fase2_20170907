...
func EjecutarRmgrp(name string, id string) {
	...
	if sesionRoot {
		...
		if name != "" && id != "" {
						...
						if GrupoYaExiste := ExisteGrupo(name, id); GrupoYaExiste {
							...
							if Existe, Indice := ExisteParticion(PathAux, NameAux); Existe {
								...
								if SB1.MontajesCount > 0 {
									...
									color.Printf("@{w}El grupo @{w}%v @{w}fue eliminado\n", name)
			...
			color.Println("@{r}	Faltan parámetros obligatorios para la COMANDO Rmgrp.")
		}

	} else {
		color.Println("@{r}Se necesita de una sesión root activa para ejecutar la COMANDO RMGRP.")
	}
