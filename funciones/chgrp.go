			...
			if len(name) <= 10 && len(grupo) <= 10 {
				...
				if IDYaRegistrado(id) {
					...
					if name != "root" {
							...
							if GrupoYaExiste := ExisteGrupo(grupo, id); GrupoYaExiste {
								...
								if Existe, Indice := ExisteParticion(PathAux, NameAux); Existe {
									...
									if SB1.MontajesCount > 0 {
										...
										color.Printf("@{w}El usuario @{w}%v @{w}fue cambiado exitosamente al grupo @{w}%v\n", name, grupo)
										...
										color.Println("@{r} La partición no ha sido formateada.")
									}
								...
								color.Printf("@{r}No existe ningun grupo registrado con el nombre @{w}%v\n", grupo)
							}
						...
						color.Printf("@{r}El sistema no permite cambiar de grupo al usuario @{w}%v\n", name)
					}
				...
				color.Println("@{r}	Los parametros no deben tener más 10 caracteres.")
			}
