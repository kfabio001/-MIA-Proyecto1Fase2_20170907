		...
		if grupo != "" && id != "" {
			...
			if len(grupo) <= 10 {
				...
				if GrupoYaExiste := ExisteGrupo(grupo, id); !GrupoYaExiste {
					...
					if IDYaRegistrado(id) {
						...
						if Existe, Indice := ExisteParticion(PathAux, NameAux); Existe {
							...
							if SB1.MontajesCount > 0 {
								...
								color.Printf("@{w}El grupo @{w}%v @{w}fue creado\n", grupo)
					...
					color.Printf("@{r}Ya hay un grupo registrado con el nombre @{w}%v\n", grupo)
				}
			...
			color.Println("@{r}	Faltan parámetros obligatorios para la COMANDO Mkgrp.")
		}

	} else {
		color.Println("@{r}Se necesita de una sesión root activa para ejecutar la COMANDO MKGRP.")
	}
...
func NumeroGrupos(id string) int {
	...
	if Existe, Indice := ExisteParticion(PathAux, NameAux); Existe {
		...
		fileMBR, err := os.Open(PathAux)
		if err != nil { //validar que no sea nulo.falta
			panic(err)
		}
